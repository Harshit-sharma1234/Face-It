...
class SimpleFaceRecognitionModule:
        ...
        for (x, y, w, h) in faces:
            ...
            if len(self.known_face_encodings) > 0:
                ...
                for i, known_encoding in enumerate(self.known_face_encodings):
                    ...
                    ssim_score = 1.0 / (1.0 + mse / 10000.0)  # Normalize MSE
                    ...
                    # Debug: Print matching scores (only if very low scores)
                    if combined_score < 0.1:
                        print(f"Low score with {self.known_face_names[i]}: template={template_score:.3f}, hist={hist_score:.3f}, ssim={ssim_score:.3f}, edge={edge_score:.3f}, combined={combined_score:.3f}")
                ...
                if best_match is not None:
                    ...
                    print(f"✅ Recognized: {name} (score: {best_score:.3f})")
                    
                    # Track student for attendance
                    if student_id:
                        self.track_student_attendance(student_id, name)
                elif len(self.known_face_encodings) > 0 and best_score > 0.01:  # Fallback for very low scores
                    # If we have a best match but it's below threshold, still use it if it's the only option
                    best_match = 0  # Use the first (and likely only) known face
                    name = self.known_face_names[best_match]
                    student_id = self.known_face_ids[best_match]
                    print(f"✅ Recognized (fallback): {name} (score: {best_score:.3f})")
                    
                    # Track student for attendance
                    if student_id:
                        self.track_student_attendance(student_id, name)
                else:
                    print(f"❌ Unknown person (best score: {best_score:.3f}, threshold: {self.matching_threshold})")
    ...
    def track_student_attendance(self, student_id: int, name: str):
        ...
        if student_id in self.present_students:
            ...
            # Student is not marked present yet - increment entry frames
            self.student_tracking[student_id]['entry_frames'] += 1
            self.student_tracking[student_id]['exit_frames'] = 0
            
        # Debug: Print tracking status
        tracking = self.student_tracking[student_id]
        print(f"Tracking {name}: entry_frames={tracking['entry_frames']}, present={student_id in self.present_students}")
    ...
    def get_attendance_updates(self) -> List[Dict]:
        ...
        for student_id, tracking in self.student_tracking.items():
            ...
            if (tracking['entry_frames'] >= self.entry_threshold and 
                ...
                print(f"🎉 {tracking['name']} marked as PRESENT! (entry_frames: {tracking['entry_frames']})")
                updates.append({
            ...
            # Check for exit (not seen for several frames)
                ...
                print(f"👋 {tracking['name']} marked as ABSENT! (exit_frames: {tracking['exit_frames']})")
                updates.append({
                    'student_id': student_id,
                    'name': tracking['name'],
                    'action': 'exited',
